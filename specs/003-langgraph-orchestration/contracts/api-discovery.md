...
`discoveryRunId` (top-level and per result): Per-run identifier for 006 dismissals scoping. Same value for all results in a run. API uses camelCase; internal/DB uses snake_case (e.g. discovery_run_id).

`coachTake` (optional): ROI micro-summary for 006 Match Inbox "Coach's Take" (per 005). Generated by Coach_Prioritization or derived from trustReport. When null, 006 may fall back to trustReport.
